...
extension Meals {
    func strIngredients() -> [String] {
       
        var rtnValue = [String]()
        let imageBaseUrl = "https://www.themealdb.com/images/ingredients/"
        if let strIngredient1 = strIngredient1, strIngredient1.count > 0 {
            let urlString = imageBaseUrl + strIngredient1 + ".png"
            rtnValue.append(urlString)
        }
        if let strIngredient2 = strIngredient2, strIngredient2.count > 0 {
            let urlString = imageBaseUrl + strIngredient2 + ".png"
            rtnValue.append(urlString)
        }
...
extension Meals {
    var ingredients: [String] {
            ...
            for i in 1...20 {
                ...
                if let value = mirror.children.first(where: { $0.label == key })?.value as? String, !value.isEmpty {
